...
public class BorrowerController {
    ...
    @DeleteMapping("/{id}")
    public ResponseEntity<Object> deleteBorrower(@PathVariable UUID id) {
        CheckAuthentication checkAuthentication = new CheckAuthentication();
        checkAuthentication.checkAuthenticationRole("ADMIN");
        try {
            borrowerService.deleteBorrowerByID(id);
            return ResponseEntity.ok().body(Collections.singletonMap("message", "Borrower deleted successfully"));
        } catch (Exception e) {
            return ResponseEntity.badRequest().body(Collections.singletonMap("error", "Error deleted borrower: " + e.getMessage()));
        }
    }

    @PutMapping("/{id}")
    public ResponseEntity<Object> updateBorrower(@PathVariable UUID id, @RequestBody BorrowerDTO updatedBorrower) {
        CheckAuthentication checkAuthentication = new CheckAuthentication();
        checkAuthentication.checkAuthenticationRole("ADMIN");
        try {
            borrowerService.updateBorrower(id, updatedBorrower);
            return ResponseEntity.ok().body(Collections.singletonMap("message", "Borrower updated successfully"));
        } catch (Exception e) {
            return ResponseEntity.badRequest().body(Collections.singletonMap("error", "Error updating borrower: " + e.getMessage()));
        }
    }
}